...
contract ConfigureOneHourEpoch is Script {
 address constant OLD=0x67881EC6Af7cCFa347fC09FF1D97Fe86b419e0fc;
 ...
 function run() external {
  ...
  require(q.epochDuration==1 days,"queue changed; review");
  ...
  address[] memory targets=new address[](4);uint256[] memory values=new uint256[](4);bytes[] memory calls=new bytes[](4);
  targets[0]=address(next);calls[0]=abi.encodeCall(GlobalConfig.setVaultQueueOverride,(CORE,GlobalConfig.QueueConfig(q.maxClaimsPerUserPerEpoch,q.cooldownPerClaim,1 hours)));
  ...
  IParamsProvider.WithdrawalParams memory w=IParamsProvider(OLD).getWithdrawalParams(CORE);
  targets[3]=address(next);calls[3]=abi.encodeCall(GlobalConfig.setVaultWithdrawalOverride,(CORE,GlobalConfig.WithdrawalConfig(w.capPerEpochBps,w.maxWithdrawalPerBlock,w.maxWithdrawalPerTx,1000,w.lockPeriod)));
  bytes32 salt=keccak256(abi.encode("MultyrOneHourEpochAndDustClaim",address(next)));
  vm.startBroadcast(key);
  ...
  require(afterQ.epochDuration==1 hours&&afterQ.maxClaimsPerUserPerEpoch==q.maxClaimsPerUserPerEpoch&&afterQ.cooldownPerClaim==q.cooldownPerClaim,"queue postcondition failed");
  require(next.getWithdrawalParams(CORE).minClaimAmount==1000,"minimum claim mismatch");
  console2.log("Minimum claim raw USDC",next.getWithdrawalParams(CORE).minClaimAmount);
